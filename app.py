...
from alpaca.data.requests import (
    StockBarsRequest, StockLatestQuoteRequest, StockQuotesRequest, StockTradesRequest
)
...
def evaluate_limit_guard(
    *,
    ...
    try:
        client = md_client()
        latest = client.get_stock_latest_quote(StockLatestQuoteRequest(symbol_or_symbols=symbol, feed=feed))
...
def _http_error_detail(status: int, body: str) -> HTTPException:
    detail = body or f"HTTP {status}"
    return HTTPException(status_code=status, detail=detail)


async def _request_with_retry(session: aiohttp.ClientSession, method: str, url: str, **kwargs: Any) -> aiohttp.ClientResponse:
    attempts = 0
    ...
    while True:
        ...
        if attempts >= max_attempts:
            raise _http_error_detail(429, body or "rate limited")
...
def _order_payload_from_model(order: BaseModel | Dict[str, Any]) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession(headers=_alpaca_headers()) as session:
        ...
        try:
            if response.status >= 400:
                ...
                raise HTTPException(status_code=response.status, detail=detail or "order rejected")
            try:
...
async def account_get(
    ...
    _require_gateway_key(x_api_key, api_key)
    async with aiohttp.ClientSession(headers=_alpaca_headers()) as session:
        async with session.get(f"{API_BASE_URL}/v2/account") as r:
            body = await r.text()
            if r.status >= 400:
...
async def orders_list(
    ...
    async with aiohttp.ClientSession(headers=_alpaca_headers()) as session:
        async with session.get(
            f"{API_BASE_URL}/v2/orders", params=params
        ) as r:
            body = await r.text()
            if r.status >= 400:
                raise HTTPException(status_code=r.status, detail=body)
...
async def positions_list_v2(
    ...
    _require_gateway_key(x_api_key, api_key)
    async with aiohttp.ClientSession(headers=_alpaca_headers()) as session:
        async with session.get(f"{API_BASE_URL}/v2/positions") as r:
            body = await r.text()
            if r.status >= 400:
                raise HTTPException(status_code=r.status, detail=body)
            return await r.json()
...
async def positions_get(
    ...
    async with aiohttp.ClientSession(headers=_alpaca_headers()) as session:
        async with session.get(
            f"{API_BASE_URL}/v2/positions/{symbol}"
        ) as r:
            body = await r.text()
            if r.status >= 400:
                raise HTTPException(status_code=r.status, detail=body)
...
async def positions_close_all(
    ...
    async with aiohttp.ClientSession(headers=_alpaca_headers()) as session:
        async with session.delete(
            f"{API_BASE_URL}/v2/positions", params=params
        ) as r:
            body = await r.text()
            if r.status >= 400:
                raise HTTPException(status_code=r.status, detail=body)
...
async def positions_close(
    ...
    async with aiohttp.ClientSession(headers=_alpaca_headers()) as session:
        async with session.delete(
            f"{API_BASE_URL}/v2/positions/{symbol}", params=params
        ) as r:
            body = await r.text()
            if r.status >= 400:
                raise HTTPException(status_code=r.status, detail=body)
